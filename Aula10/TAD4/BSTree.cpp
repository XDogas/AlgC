...
// A: Sim, porque antes de o nó pai ser destruído, são destroídos os nós filhos, não sendo criadas variáveis temporárias.
...
static void* _treeGetMax(const struct _BSTreeNode* root) {  //5
  assert(root != NULL);

  // Complete a função com uma solução RECURSIVA.
  if(root->right == NULL) return root->item;
  if(root->right != NULL) return _treeGetMax(root->right);
...
int BSTreeContains(const BSTree* header, const void* item) {  //
  ...
  while (root != NULL) {
    if(header->compare(item,root->item) == 0) return 1;
    else if(header->compare(item,root->item) > 0) root = root->right;
    else root = root->left;
  }
  return 0;
}
...
_treeTraverseINOrder(struct _BSTreeNode* root, void (*function)(void* p)) { //
  // Os nós são visitados por ordem crescente.
  if (root == NULL) return;
  
  if(root->left != NULL) _treeTraverseINOrder(root->left, function);

  function(root->item);

  if(root->right != NULL) _treeTraverseINOrder(root->right, function);  
}
...
int BSTreeAdd(BSTree* header, void* item) { //
  ...
  // Ancoragem do novo nó no ramo adequado.
  if(root != NULL){
	  if(cmp > 0) prev->right = newNode;
	  else prev->left = newNode;
  }
  else header->root = newNode;
...
// R: A complexidade computacional das operações é logarítmica, visto que a árvore é
//    equilibrada, ou seja, o grau de complexidade da função pertence a O(log(n)).
// 
// Explique como poderia evitar a chamada a _getNumNodes nesta função.
// Q: O que seria preciso mudar na estrutura de dados?
// R: Seria evitada a chamada desta função se cada nó tivesse a informação da sua posiçao na árvore.
//
// Q: E que funções teriam de ser modificadas?
// R: Teriam de ser modificadas as funções: BSTAdd, BSTRemove, BSTreeCreate e BSTDestroy.
//
...
// R: Percorrendo a árvore até encontrar o elemento, uma vez encontrado,
//    esse elemento era eliminado, não chamando a função BSTreeRemove.
//
