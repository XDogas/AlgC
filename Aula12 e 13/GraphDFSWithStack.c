...
GraphDFSWithStack* GraphDFSWithStackExecute(Graph* g,
  ...
  while(!StackIsEmpty(stack)){
    ...
    for (int i = 0; i < sizeof(adjacents) / sizeof(unsigned int); i++) {

      if(!traversal->marked[adjacents[i]]){

        traversal->predecessor[adjacents[i]] = startVertex;
        StackPush(stack,adjacents[i]);
...
void GraphDFSWithStackDisplay(const GraphDFSWithStack* p) {
  ...
  for(int i = 0; i < GraphGetNumVertices(p->graph); i++) GraphDFSWithStackShowPath(p,i);  
}
