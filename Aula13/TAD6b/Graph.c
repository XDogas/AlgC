...
Graph* GraphCopy(const Graph* g) {
  ...
  if(g->isComplete) graph = GraphCreateComplete(numVertices,isDigraph);
  else graph = GraphCreate(numVertices,isDigraph,isWeighted);
...
unsigned short GraphCheckInvariants(const Graph* g) {
  ...
  while(ListGetCurrentPos(g->verticesList) < (ListGetSize(g->verticesList) - 1)) {

    int num_edges_vertice = ListGetSize(ListGetCurrentItem(g->verticesList));
    int num_edges = 0;    
    num_edges += num_edges_vertice;
